...
graphics_variable_dict = input_file_to_variable_dict('graphics_description_solovev.dat')
...
delta = 0.01
r = np.arange(xmin, xmax, delta)
z = np.arange(zmin, zmax, delta)
R, Z = np.meshgrid(r, z)

rmaj = float(graphics_variable_dict['rmaj'])
kappa = float(graphics_variable_dict['kappa'])
outer_bound = float(graphics_variable_dict['outer_bound'])

psi_grid = psi(R, Z, rmaj, kappa, outer_bound)
fig = plt.gcf()
ax = fig.gca()
CS = ax.contour(R, Z, psi_grid, [1.0])
ax.clabel(CS, levels=[])
