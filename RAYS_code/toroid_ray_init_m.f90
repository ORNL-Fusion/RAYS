...
! Data for initial k launch using a beam of rays.  This requires some explanation:
! For each of the launch positons above there is a beam.  This is a bit unrealistic since, 
! at least for now, the beam location needs to be inside the plasma.  The axis of the beam
! is defined by thetaB = angle wrt equatorial plane, and phiB = angle wrt toroidal direction.
! The initial ray direction relative to the beam orientation is defined by rindex_theta and
! rindex_phi.  So at least for now the beam is circular cross section.
! N.B. Actually beam is not implemented yet but here is the data declaration for when it is.

    integer n_thetaB, n_phiB  ! Beam axis orientation
    real thetaB0, dthetaB, phiB0, dphiB 

    integer n_thetaR, n_phiR  ! Ray distribution around beam axis
    real thetaR0, dthetaR, phiR0, dphiR
     
 namelist /toroid_ray_init_list/ &
     ...
     & n_thetaB, thetaB0, dthetaB, n_phiB, phiB0, n_phiB, &    
     & n_thetaR, thetaR0, dthetaR, n_phiR, phiR0, n_phiR
...
!****************************************************************************
    ...
    subroutine ray_init_toroid_kphi_ktheta(nray_max, nray, rvec0, rindex_vec0)
...
! Read and write input namelist
    ...
    read(input_unit, toroid_ray_init_list)
    close(unit=input_unit)
    write(message_unit, toroid_ray_init_list)
...
! Calculate a bunch of unit vectors, parallel and transverse components of k
        psi_unit = eq%gradpsi
        psi_unit = psi_unit/sqrt(dot_product(psi_unit,psi_unit))

        phi_unit = (/ 0._rkind, 1._rkind, 0._rkind/)
        
        theta_unit = (/eq%gradpsi(3), 0._rkind, -eq%gradpsi(1) /) ! psi_unit X phi_unit
...
! Solve dispersion for complex refractive index in psi direction            
    ...
    end subroutine ray_init_toroid_kphi_ktheta
                                             
!****************************************************************************

!N.B. there is partial coding for ray_init_toroid_beam in the old files from Cai-Ye Wang
! 1995.  Also there are ray weight function for various launch geometries.  But in the
! present incarnation these would be delegated to a post processing routine.

! e.g.  subroutine ray_init_toroid_beam 
    
!***********************************************************************************



end module toroid_ray_init_m
