...
! fraction of the electorn density given by n0s(0:nspec).  In the future, when ion dynamics
...
! namelist /density_spline_interp_list/ variables
	...
	real(KIND=rkind) ::  ne_in(n_grid_max) ! Values on grid
...
!****************************************************************************************
  ...
  subroutine  density_spline_interp(psi, dens, dd_psi)
 ...
 write(*,*) 'density_spline_interp: got to 1'

    call ne_profile_N%eval_1D_fp(psi, dens, dd_psi)
 write(*,*) 'density_spline_interp: got to 2'
